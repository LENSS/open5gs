...
char *gen_proxy_key(unsigned int from_ip, uint16_t from_port, unsigned int to_ip, uint16_t to_port) {
    ...
    sprintf(key, "%d:%d:%d:%d", from_ip, from_port, to_ip, to_port);
    return key;
}
...
int handle_packet(uint8_t *data, int len, char *err, int err_buf_len)
{
    ...
    struct proxy_t *proxied = ogs_hash_get(proxy_hash, proxy_key, strlen(proxy_key));
    if (proxied != NULL)
    {
    ...
    opts = convert_parse_tlvs(data + CONVERT_HDR_LEN, tlv_len);
    if (opts == NULL)
    {
